function outT = least_error(histo, startT)
% Calculates a threshold value from a histogram using the mid-way method. 
%
% histo: A 1D array histogram. 
% The histogram bins must correspond to gray value 0, 1, 2,...
% startT: start threshold
% outT: the calculated threshold (integer)
%
% Programmed by Maria Magnusson 2018-08

num = length(histo);
t0 = floor(startT);
t1 = t0+2;

% Calculate the threshold
%------------------------
while abs(t0-t1) > 0.5

  t0 = t1;
    
  % Calculate mean for the lower part of the histogram
  %---------------------------------------------------
  lowersum1 = sum(histo(1:t0+1).*[0:t0]);
  lowersum2 = sum(histo(1:t0+1));
   
  lowersum1_leasterror = sum(histo(1:t0+1));
  lowersum2_leasterror = sum(histo(1:num));
  
  lowersum2_leasterror_var = sum(histo(1:t0+1));
  
  if lowersum2 ~= 0
    mean0 = lowersum1/lowersum2;
    lowersum1_leasterror_var = sum(histo(1:t0+1).*([0:t0]-mean0).^2);
    P0 = lowersum1_leasterror/lowersum2_leasterror;
    var0 = lowersum1_leasterror_var/lowersum2_leasterror_var;
    %p0 = (P0/sqrt(2*pi*var0))*exp(-1/2*(([0:t0]-mean0)^2/var0));
  else
    error('Cannot calculate new threshold');
  end;
    
  % Calculate mean for the upper part of the histogram
  %---------------------------------------------------
  uppersum1 = sum(histo(t0+2:num).*[t0+1:num-1]);
  uppersum2 = sum(histo(t0+2:num));
  
  uppersum1_leasterror = sum(histo(t0+2:num));
  uppersum2_leasterror = sum(histo(1:num));
  
  uppersum2_leasterror_var = sum(histo(t0+2:num));
  
  if uppersum2 ~= 0
    mean1 = uppersum1/uppersum2;
    uppersum1_leasterror_var = sum(histo(t0+2:num).*([t0+1:num-1]-mean1).^2);
    P1 = uppersum1_leasterror/uppersum2_leasterror;
    var1 = uppersum1_leasterror_var/uppersum2_leasterror_var;
    %p1 = (P1/sqrt(2*pi*var1))*exp(-1/2*(([t0+1:num-1]-mean1)^2/var1));
  else
    error('Cannot calculate new threshold');
  end;

  % Calculate new threshold
  %-----------------------

  a = (var0*var1)/(var1-var0);
  b = 2*((mean1/var1)-(mean0/var0));
  c = -2*log(P0/P1) + log(var0/var1) + mean0^2/var0 - mean1^2/var1;
  T = [1 a*b a*c];
  Tout = roots(T)
  if(mean0 <= Tout(1) && Tout(1) <= mean1)
    t1 = Tout(1)
  elseif(mean0 <= Tout(2) && Tout(2) <= mean1)
    t1 = Tout(2)
  else
      error('HELP')
  end
  t1 = floor(t1)
  
end;

outT = t1;